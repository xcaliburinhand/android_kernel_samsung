#!/bin/sh

#[[ -d release ]] || {
#	echo "must be in kernel root dir"
#	exit 1;
#}

echo "packaging it up"

RELVER=$1

REL=ICS_Captivate_$(date +%Y%m%d_r)${RELVER}_update.zip

rm -r release/system/lib/modules 2> /dev/null
mkdir -p release/system/lib/modules || exit 1
#cp stable/* release/system/lib/modules/
find . -name "*.ko" -exec cp {} release/system/lib/modules/ \; 2>/dev/null || exit 1

cd release && {
	zip -q -r ${REL} system boot.img META-INF bml_over_mtd bml_over_mtd.sh busybox || exit 1
#	sha256sum ${REL} > ${REL}.sha256sum
} || exit 1

rm system/lib/modules/*
echo ${REL}
exit 0
